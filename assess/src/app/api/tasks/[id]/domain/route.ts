...
export async function PUT(
  ...
  try {
    ...
    await prisma.$transaction([
      prisma.question.deleteMany({ where: { domainTaskId: taskId } }),
    ...
    return NextResponse.json({ success: true, message: "Domain questions updated." });
  } catch {
    return NextResponse.json({ error: "Failed to update domain questions." }, { status: 500 });
  }
}
